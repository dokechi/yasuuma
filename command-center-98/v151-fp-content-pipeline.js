(()=>{
  ...
  const DESIGN_DIRECTIONS={
    friendly:{label:'親しみやすい整理型',short:'今の良さを整える',description:'大きな問いと数字を主役にし、強弱と余白を整える。付箋と小さな線画を使う。'},
  ...
  const screenshotRow=(payload,row)=>{
    const value=screenshotDecision(payload,row);
    return '<article class="fp-shot '+(row?.required?'required':'optional')+'"><div><b>'+(row?.required?'必須':'あると良い')+'</b><strong>'+escFp(row?.label||'スクショ候補')+'</strong></div>'+(row?.url?'<a href="'+escFp(row.url)+'" target="_blank" rel="noopener">撮影元を開く</a>':'')+'<dl><dt>撮る範囲</dt><dd>'+escFp(row?.capture_range||row?.capture_area||'')+'</dd><dt>使う理由</dt><dd>'+escFp(row?.purpose||'')+'</dd>'+(row?.slide_no?'<dt>使用ページ</dt><dd>'+escFp(row.slide_no)+'</dd>':'')+(row?.acquisition_note?'<dt>取得メモ</dt><dd>'+escFp(row.acquisition_note)+'</dd>':'')+'</dl><label>画像の用意<select data-fp-shot-decision="'+escFp(row?.id||'')+'"><option value="">選んでください</option><option value="assistant"'+selected(value,'assistant')+'>まずAIが取得を試す</option><option value="user"'+selected(value,'user')+'>自分でスクショを用意</option>'+(row?.required?'':'<option value="skip"'+selected(value,'skip')+'>今回は使わない</option>')+'</select></label></article>';
  };
  const preflightSection=(item,payload)=>{
    ...
    return '<section class="fp-preflight"><h4>制作前に決める</h4><p>同じ原稿のまま、見せ方と実物素材の扱いだけを先に固定します。</p><div class="fp-design-options">'
      ...
      +'<div class="fp-shot-list">'+(screenshots.length?screenshots.map(row=>screenshotRow(payload,row)).join(''):'<p class="fp-empty">今回はスクショ素材なし。デザインだけ選べば進めます。</p>')+'</div>'
  ...
  const savePreflight=async(item,button)=>{
    ...
    list(item.payload?.screenshot_requests).forEach(row=>{
      const value=screenshotDecisions[row?.id],allowed=row?.required?['assistant','user']:['assistant','user','skip'];
